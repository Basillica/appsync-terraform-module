...
variable "authentication_type" {
  description = "The authentication type to use by GraphQL API. Could be AWS_IAM, API_KEY, OPENID_CONNECT, AMAZON_COGNITO_USER_POOLS etc."
  type        = string
  default     = "API_KEY"
}
...
variable "log_cloudwatch_logs_role_arn" {
  description = "ARN of the role that AppSync will assume to publish to cloudWatch."
  type        = string
  default     = null
}
...
variable "graphql_api_tags" {
  description = "Map of tags to add to the GraphQL API"
  type        = map(string)
  default     = {}
}
...
variable "schema_directory" {
  description = "the directory of the graphql api schema definitions"
  type = string
}

variable "resolver_path" {
  description = "the directory of the resolvers"
  type = string
}